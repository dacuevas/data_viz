...
data95 <- read.table("function_distribution_95.txt", sep="\t", header=T, check.names=F)

# Reorder data
substrateTab95 <- table(subset(data95, category == "Expected Growth", select = substrate))
...
# Plot
ggplot(data95[data95$category=="Gain of Function",],aes(x=substrate,y=clone)) + geom_point(size=5) + facet_wrap(~category,ncol=1) + 
